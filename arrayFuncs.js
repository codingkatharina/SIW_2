...
// 2 use split to create array
// 3 use join() to bring words together again
let weirdSentence = "  This|sentence|has|some|spaces|" +
                     "and|delimiters,|make|it" +
                     "|better|readable.              ";

let noSpaces = weirdSentence.trim();

function splitting(noSpaces) {
    let sentence = noSpaces.split("|");
    let sentenceJoin = sentence.join(" ");
...
for (i=0; i<100; i++) {
if (i<=50) {
    numbersTillFifty.push(i)
}
if (i>50 && i<100) {
    numbersFromFifty.push(i)
}
}

console.log(numbersTillFifty);
console.log(numbersFromFifty);
...
const backToFrontSplit = backToFront.split("");
const backToFrontReverse = backToFrontSplit.reverse();
const frontToBack = backToFrontReverse.join("");
...
numbers.splice(biggestNum);
let sortedArr = numbers.splice(4,1);
//sortedArr.unshift(biggestNum);        not needed here to get 10 into sorted Array

//console.log(biggestNum);              testing what biggest number is
console.log(numbers)                  
